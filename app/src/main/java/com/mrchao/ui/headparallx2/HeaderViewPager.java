...
public class HeaderViewPager extends LinearLayout {
    ...
    private View mOverTagView;    //对外提供一个设置图片的方法

    public void setParallaxImageView(final ImageView imageView) {
        this.mOverTagView = imageView;
        //在视图树中，设定最大高度，
    ...
    private boolean overViewIsParallaxing = false;//是否在执行视差
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                 ...
                 */
                if (verticalScrollFlag && (!isStickied() || mScrollable.isTop() || isClickHead) && !overViewIsParallaxing) {
                ...
                /**
                 * 满足垂直滑动，且已经滑倒顶部,开始拦截事件，执行视差动画，
                 * 此过程拦截事件，直到视差动画结束头部恢复了初始状态，交出事件
                 */
                if (canPtr()) {
                    if (mOverTagView != null) {
                        overViewIsParallaxing = true;
                        int newHeight = (int) (mOverTagView.getHeight() - deltaY / 3);
                        if (newHeight > maxHeight) {
                            newHeight = maxHeight;
                        }
                        if (newHeight <= orignalHeight) {
                            newHeight = orignalHeight;
                            overViewIsParallaxing = false;
                        }
                        if (overViewIsParallaxing) {
                            mOverTagView.getLayoutParams().height = newHeight;
                            mOverTagView.requestLayout();//使ImageView的布局参数生效
                        }
                    }
    ...
    private void relesetOverView() {
        overViewIsParallaxing = false;
        if (mOverTagView != null) {
            ValueAnimator animator = ValueAnimator.ofFloat(mOverTagView.getHeight(), orignalHeight);
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animator) {
                    float value = (float) animator.getAnimatedValue();
                    mOverTagView.getLayoutParams().height = (int) value;
                    mOverTagView.requestLayout();
                }
            });
